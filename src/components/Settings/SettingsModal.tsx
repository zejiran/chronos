...
type SettingsTab = 'general' | 'appearance' | 'calendars' | 'accounts' | 'notifications' | 'shortcuts' | 'advanced';
          ...
          <div class={css({ display: 'flex', flex: 1, overflow: 'hidden' })}>
                ...
                {(tab) => (
                  <button
                    ...
                    class={css({
                      ...
                      borderRadius: '0.375rem',
                      border: 'none',
                      backgroundColor: activeTab() === tab.id ? 'var(--colors-muted)' : 'transparent',
                      color: 'var(--colors-foreground)',
                      fontSize: '0.875rem',
                      textAlign: 'left',
                      cursor: 'pointer',
            ...
            >
              ...
              <Show when={activeTab() === 'shortcuts'}>
                <ShortcutsSettings settings={$settings()} onUpdate={updateSetting} />
              </Show>

              <Show when={activeTab() === 'advanced'}>
                <AdvancedSettings settings={$settings()} configPath={configPath()} onUpdate={updateSetting} />
              </Show>
            </div>
          </div>
...
function SettingRow(props: { label: string; description?: string; children: any }) {
  return (
    ...
    >
      <div>
        <div class={css({ fontSize: '0.875rem', fontWeight: '500', color: 'var(--colors-foreground)' })}>
          {props.label}
        </div>
        <Show when={props.description}>
          <div class={css({ fontSize: '0.75rem', color: 'var(--colors-foreground)', opacity: 0.6, marginTop: '0.25rem' })}>
            {props.description}
          </div>
        </Show>
      </div>
  ...
  return (
    <button
      ...
      class={css({
        ...
        height: '1.5rem',
        borderRadius: '0.75rem',
        backgroundColor: props.checked ? 'var(--colors-primary)' : 'var(--colors-muted)',
        border: 'none',
        cursor: 'pointer',
        position: 'relative',
...
function Select(props: { value: string; options: { value: string; label: string }[]; onChange: (value: string) => void }) {
  return (
    <select
      value={props.value}
    ...
    <div>
      ...
      <SettingRow label="Start of week" description="First day of the week in calendar views">
        <Select
          ...
          onChange={(v) =>
            props.onUpdate('calendar', { ...settings()?.calendar, startOfWeek: v })
          }
        />
      </SettingRow>
      ...
      <SettingRow label="Default view" description="View shown when app opens">
        <Select
          ...
          onChange={(v) =>
            props.onUpdate('calendar', { ...settings()?.calendar, defaultView: v as CalendarView })
          }
        />
      </SettingRow>
      ...
      <SettingRow label="Time format" description="12-hour or 24-hour clock">
        <Select
          ...
          onChange={(v) =>
            props.onUpdate('calendar', { ...settings()?.calendar, timeFormat: v })
          }
        />
      </SettingRow>

      <SettingRow label="Show week numbers" description="Display week numbers in calendar views">
        <Toggle
          checked={settings()?.calendar.showWeekNumbers || false}
          onChange={(v) =>
            props.onUpdate('calendar', { ...settings()?.calendar, showWeekNumbers: v })
          }
        />
      </SettingRow>

      <SettingRow label="Show declined events" description="Display events you've declined">
        <Toggle
          checked={settings()?.calendar.showDeclinedEvents || false}
          onChange={(v) =>
            props.onUpdate('calendar', { ...settings()?.calendar, showDeclinedEvents: v })
          }
        />
      </SettingRow>

      <SettingRow label="Default event duration" description="Duration for new events in minutes">
        <Select
          ...
          onChange={(v) =>
            props.onUpdate('calendar', { ...settings()?.calendar, defaultEventDuration: parseInt(v) })
          }
        />
      </SettingRow>
      ...
      <div class={css({ marginTop: '2rem' })}>
        ...
        <SettingRow label="Auto sync" description="Automatically sync calendars in background">
          <Toggle
            checked={settings()?.sync.autoSync || false}
            onChange={(v) => props.onUpdate('sync', { ...settings()?.sync, autoSync: v })}
          />
        </SettingRow>

        <SettingRow label="Sync interval" description="How often to sync calendars">
          <Select
            ...
            onChange={(v) =>
              props.onUpdate('sync', { ...settings()?.sync, syncInterval: parseInt(v) })
            }
          />
        </SettingRow>

        <SettingRow label="Conflict resolution" description="How to handle sync conflicts">
          <Select
            ...
            onChange={(v) =>
              props.onUpdate('sync', { ...settings()?.sync, conflictResolution: v })
            }
          />
        </SettingRow>
      </div>
...
function AppearanceSettings(props: { settings: Settings | null; onUpdate: any }) {
  return (
    <div>
      ...
      <div class={css({ marginTop: '2rem' })}>
        ...
        <SettingRow label="Font size" description="Base font size for the app">
          <Select
            ...
            onChange={(v) =>
              props.onUpdate('appearance', { ...props.settings?.appearance, fontSize: parseInt(v) })
            }
          />
        </SettingRow>
        ...
        <SettingRow label="UI density" description="Spacing between elements">
          <Select
            ...
            onChange={(v) =>
              props.onUpdate('appearance', { ...props.settings?.appearance, density: v })
            }
          />
        </SettingRow>
      </div>
  ...
  return (
    <div>
      ...
      <Show
        ...
        fallback={
          <div class={css({ textAlign: 'center', padding: '2rem', color: 'var(--colors-foreground)', opacity: 0.6 })}>
            ...
            {(calendar) => (
              ...
              >
                ...
                <div class={css({ flex: 1 })}>
                  ...
                  <div class={css({ fontSize: '0.75rem', color: 'var(--colors-foreground)', opacity: 0.6 })}>
...
function NotificationSettings(props: { settings: Settings | null; onUpdate: any }) {
  const settings = () => props.settings;
  ...
  return (
    <div>
      ...
      <SettingRow label="Enable notifications" description="Show desktop notifications for events">
        <Toggle
          checked={settings()?.notifications.enabled || false}
          onChange={(v) => props.onUpdate('notifications', { ...settings()?.notifications, enabled: v })}
        />
      </SettingRow>
      ...
      <SettingRow label="Sound" description="Play sound with notifications">
        <Toggle
          ...
          onChange={(v) => props.onUpdate('notifications', { ...settings()?.notifications, soundEnabled: v })}
        />
      </SettingRow>

      <SettingRow label="Show event details" description="Include event details in notification">
        <Toggle
          checked={settings()?.notifications.showEventDetails || true}
          onChange={(v) => props.onUpdate('notifications', { ...settings()?.notifications, showEventDetails: v })}
        />
      </SettingRow>
      ...
      <div class={css({ marginTop: '2rem' })}>
        ...
        <SettingRow label="Enable silent hours" description="Mute notifications during specified times">
          <Toggle
            checked={settings()?.notifications.silentHours.enabled || false}
            onChange={(v) =>
              props.onUpdate('notifications', {
                ...settings()?.notifications,
                silentHours: { ...settings()?.notifications.silentHours, enabled: v },
              })
            }
          />
        </SettingRow>
...
function ShortcutsSettings(props: { settings: Settings | null; onUpdate: any }) {
  const shortcuts = [
      ...
      <div class={css({ display: 'flex', flexDirection: 'column', gap: '0.5rem' })}>
        <For each={shortcuts}>
          {(shortcut) => (
            ...
            >
              <span class={css({ fontSize: '0.875rem', color: 'var(--colors-foreground)' })}>
                {shortcut.label}
              </span>
              <kbd
                class={css({
...
function AdvancedSettings(props: { settings: Settings | null; configPath: string; onUpdate: any }) {
  const settings = () => props.settings;
    ...
    <div>
      ...
      <SettingRow label="Developer mode" description="Enable advanced debugging features">
        <Toggle
          ...
          onChange={(v) => props.onUpdate('advanced', { ...settings()?.advanced, developerMode: v })}
        />
      </SettingRow>
      ...
      <SettingRow label="Log level" description="Verbosity of application logs">
        <Select
          ...
          onChange={(v) => props.onUpdate('advanced', { ...settings()?.advanced, logLevel: v })}
        />
      </SettingRow>
      ...
      <div class={css({ marginTop: '2rem' })}>
        ...
        <SettingRow label="Analytics" description="Help improve Chronos by sending anonymous usage data">
          <Toggle
            checked={settings()?.privacy.analyticsEnabled || false}
            onChange={(v) => props.onUpdate('privacy', { ...settings()?.privacy, analyticsEnabled: v })}
          />
        </SettingRow>
        ...
        <SettingRow label="Crash reports" description="Automatically send crash reports">
          <Toggle
            ...
            onChange={(v) => props.onUpdate('privacy', { ...settings()?.privacy, crashReportsEnabled: v })}
          />
        </SettingRow>
      </div>
      ...
      <div class={css({ marginTop: '2rem' })}>
        ...
        <div class={css({ marginTop: '1rem', display: 'flex', gap: '0.5rem' })}>
          ...
          <Button variant="ghost" size="sm" class={css({ color: 'var(--colors-error)' })}>
            Clear Cache
          </Button>
        </div>
      </div>
      ...
      <div class={css({ marginTop: '2rem' })}>
        ...
        <div class={css({ fontSize: '0.875rem', color: 'var(--colors-foreground)', opacity: 0.6 })}>
          <p>Chronos Calendar v{settings()?.version || '1.0.0'}</p>
          <p class={css({ marginTop: '0.5rem' })}>A modern, cross-platform calendar application.</p>
        </div>
      </div>
    </div>
  );
