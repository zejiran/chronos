...
import { createEvent, updateEvent as updateEventApi, deleteEvent as deleteEventApi } from '../../lib/tauri';
...
function formDataToEvent(data: EventFormData, existingEvent?: CalendarEvent): Omit<CalendarEvent, 'id' | 'createdAt' | 'updatedAt'> {
...
export function EventModal() {
  ...
  const [formData, setFormData] = createSignal<EventFormData>(getDefaultFormData());
  ...
  createEffect(() => {
    if ($isOpen()) {
      ...
      if (event) {
        ...
        const primary = calendarsList.find(c => c.isPrimary) || calendarsList[0];
        if (primary) {
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      const eventData = formDataToEvent(formData(), $selectedEvent() || undefined);
      ...
      if (isEditing()) {
        ...
        updateEvent($selectedEvent()!.id, updated);
      } else {
  ...
  const recurrenceOptions: { value: RecurrenceRule['frequency'] | null; label: string }[] = [
          ...
          <form onSubmit={handleSubmit} class={css({ padding: '1.5rem' })}>
            <div class={css({ display: 'flex', flexDirection: 'column', gap: '1rem' })}>
              ...
              <div>
                ...
                <select
                  value={formData().reminderMinutes === null ? 'null' : formData().reminderMinutes}
                  onChange={(e) =>
                    updateField(
                      'reminderMinutes',
                      e.currentTarget.value === 'null' ? null : parseInt(e.currentTarget.value),
                    )
                  }
                  class={css({
                    width: '100%',
            ...
            >
              ...
              <div class={css({ display: 'flex', gap: '0.5rem', marginLeft: 'auto' })}>
